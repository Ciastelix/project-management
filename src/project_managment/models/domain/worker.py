...
class Worker(Base):
    ...
    role = Column(SQLAlchemyEnum(*RoleEnum.__members__), default=RoleEnum.WORKER)
    location = Column(String, nullable=False)
    is_active = Column(Boolean, default=True)
    is_available = Column(Boolean, default=True)
    user = relationship('User', backref='worker', uselist=False)
    project_workers = relationship('Project', secondary=worker_project, backref="worker_projects")
