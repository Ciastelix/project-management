...
@inject
def create_user(user: UserInCreate, user_service: UserService = Depends(Provide[Container.user_service])):
    return user_service.add(user)

@router.get("/{user_id}", response_model=UserInResponse, status_code=status.HTTP_200_OK)
@inject
def read_user(user_id: UUID, user_service: UserService = Depends(Provide[Container.user_service])):
    return user_service.get_by_id(user_id)

@router.put("/{user_id}", response_model=UserInResponse, status_code=status.HTTP_200_OK)
@inject
def update_user(user_id: UUID, user: UserInUpdate, user_service: UserService = Depends(Provide[Container.user_service])):
    return user_service.update(user_id, user)

@router.delete("/{user_id}", status_code=status.HTTP_204_NO_CONTENT)
@inject
def delete_user(user_id: UUID, user_service: UserService = Depends(Provide[Container.user_service])):
